...
def main():
    ...
    parser.add_argument('-o', '--output-file', help="Output file", default="Samples.dox")
    args = parser.parse_args()

    with open(args.output_file, 'w') as f:
        f.write(f"File generate using `python3 {' '.join(sys.argv)}`")
        f.write(fr"""/**
\page {args.output_file.replace('.dox', '')} {args.output_file.replace('.dox', '').replace('_', ' ')}
""")
        pages = []
        for sample in sorted(glob(join(args.sample_folder, '*'))):
            ...
            if isfile(file):
                pages.append({
                    'file': file,
                    'page': args.output_file.replace('.dox', '') + '_' + basename(sample),
